...
describe('fish', () => {
	...
	it('/f 可以触发抛竿', async () => { vi.mocked(coinService.getBalance).mockResolvedValue(50); await handleFish(makeParsed({ command: 'f', args: ['3'] }), { COIN_DO: {} as any, FISH_KV: makeKv(), FISHING_RECORD_KV: { get: vi.fn().mockResolvedValue(null), put: vi.fn().mockResolvedValue(undefined) } } as any); expect(coinService.addToTreasury).toHaveBeenCalled(); });
	...
	it('禁止房间（默认群的非许可主题）', async () => { await handleFish(makeParsed({ threadId: 999, args: ['3'] }), {} as any); expect(vi.mocked(TgMessage.sendText)).not.toHaveBeenCalled(); });
	...
	it('/钓鱼 不触发钓鱼抛竿逻辑', async () => { vi.mocked(coinService.getBalance).mockResolvedValue(50); await handleFish(makeParsed({ command: '钓鱼', args: ['3'] }), { COIN_DO: {} as any, FISH_KV: makeKv(), FISHING_RECORD_KV: { get: vi.fn().mockResolvedValue(null), put: vi.fn().mockResolvedValue(undefined) } } as any); expect(coinService.addToTreasury).not.toHaveBeenCalled(); });
