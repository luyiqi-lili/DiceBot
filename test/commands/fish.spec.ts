...
describe('fish', () => {
	...
	it('/钓鱼 不触发 /fish 抛竿逻辑', async () => { vi.mocked(coinService.getBalance).mockResolvedValue(50); await handleFish(makeParsed({ command: '钓鱼', args: ['3'] }), { COIN_DO: {} as any, FISH_KV: makeKv(), FISHING_RECORD_KV: { get: vi.fn().mockResolvedValue(null), put: vi.fn().mockResolvedValue(undefined) } } as any); expect(coinService.addToTreasury).not.toHaveBeenCalled(); });
