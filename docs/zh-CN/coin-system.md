...
用户之间转账不收取手续费。接收方会收到完整转账金额，国库不会从转账中收取任何款项。

## 祈祷

已知设有“神殿”主题的群组，所有话题（包括一般话题及新建话题）的普通文字发言都可以自动签到，不受祈祷话题权限设置限制。奖励为随机 15–20 coin，通知固定发送到神殿并显示奖励及余额。同群的 `/coin pray` 也可在任何话题使用，通知同样只发到神殿，重复签到保持静默。其他群保留原有手动祈祷话题限制。每天香港时间早上 8:00 开始新的签到日。
