...
describe('AI Gateway Gemini translation', () => {
	it('uses Google native Gemini through the Worker-bound AI Gateway', async () => {
		...
		expect(result).toEqual({ status: 'ok', text: 'Hello, world!', provider: 'gemini-gateway' });
	...
	it('fails closed without making a network request when Gateway secrets are absent', async () => {
		const fetchFn = vi.fn();
		const result = await translateWithGemini({} as any, {
	...
	it('falls back to the legacy JSON Google key pool after a rejected direct key', async () => {
		...
		expect(result).toEqual({ status: 'ok', text: 'Hello', provider: 'gemini-gateway' });
	...
	it('falls back to Google directly when the Gateway rejects a valid key', async () => {
		...
		expect(result).toEqual({ status: 'ok', text: 'Hello', provider: 'gemini-direct' });
	...
	it('falls back to DeepSeek after Google rejects every production key path', async () => {
		const fetchFn = vi.fn()
			.mockResolvedValueOnce(new Response('gateway denied', { status: 403 }))
			.mockResolvedValueOnce(new Response('direct denied', { status: 403 }))
			.mockResolvedValueOnce(new Response(JSON.stringify({
		...
		const result = await translateWithGemini({
			AI: { gateway, run } as any,
			AI_GATEWAY_TOKEN: 'gateway-run-token',
			GOOGLE_API_KEY: 'google-key',
			DEEPSEEK_API_KEY: 'deepseek-key',
		} as any, { targetLanguage: 'English', text: '你好' }, { fetchFn });

		expect(result).toEqual({ status: 'ok', text: 'Hello', provider: 'deepseek' });
		expect(fetchFn.mock.calls[2][0]).toBe('https://api.deepseek.com/chat/completions');
		expect(fetchFn.mock.calls[2][1].headers.Authorization).toBe('Bearer deepseek-key');
