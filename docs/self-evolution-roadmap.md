...
`DONATION_ADMIN_KEY` protects metadata listing, validation, disable, and revoke operations; production currently leaves those admin APIs disabled by not configuring it. Donor-owned `/revoketoken` remains available and deletes the Secrets Store key before marking metadata revoked. Gemini validation calls a read-only model-list endpoint through Gateway. Ollama caches the model list and performs a fixed one-token authenticated inference probe so invalid keys cannot pass on a public catalog response alone. Because custom-provider BYOK aliases were observed not to inject upstream Authorization, production reads the activated Ollama key only through a matching Cloudflare Secrets Store Worker binding and adds that header to the AI Gateway request; no key value is stored locally or in D1. A managed DeepSeek alias records the supported premium-model catalog without reading the key back; only legacy encrypted DeepSeek records can call the direct balance endpoint. One shared credential can be health-checked per hourly run.
...
The decision first round-robins donated Ollama Cloud aliases and selects a large model from each validated OpenAI-compatible `/v1/models` catalog. It falls back to Workers AI `@cf/qwen/qwen3.8-27b`. Both paths use AI Gateway and must return `risk=low` with confidence at or above `GITHUB_AI_TRIAGE_MIN_CONFIDENCE` (production: `0.85`). Translation uses the completely-free small-model pool: donated Gemini first, then Ollama Cloud small models, then Workers AI `@cf/meta/llama-3.2-3b-instruct`.
