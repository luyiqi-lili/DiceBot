...
- `GITHUB_ISSUE_INTAKE_ENABLED`：默认关闭的公开 Issue 写开关；仓库中为 `false`。
...
- `GITHUB_ISSUE_TOKEN`：仅供 `/wish`、`/issue` 创建 Issue 的仓库级 Issues 写 token。
...
- Cloudflare Worker：`TOKEN`、`EXTERNAL_API_KEY`、`DONATION_INTAKE_KEY`、`DONATION_ADMIN_KEY`、`DONATION_ENCRYPTION_KEY`、`GITHUB_TOKEN` 与 `GITHUB_ISSUE_TOKEN`。本地 `.env` 的 `GH_TOKEN` 写入 Cloudflare 时映射为只读扫描使用的 `GITHUB_TOKEN`；除非权限范围明确经过设计，否则不要复用成 Issue 写 token。
...
先执行 `schema/d1.sql` 并配置好独立的写/管理 token，再把 `GITHUB_ISSUE_INTAKE_ENABLED` 改为 `true`。仓库默认值刻意保持关闭。
