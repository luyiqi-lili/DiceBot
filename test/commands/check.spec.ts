...
describe('/check（本地静态规则查询，无外部 AI）', () => {
	...
	it('回答每日签到周年庆 50c 触发逻辑', async () => {
		...
		expect(reply?.text).toContain('2026-06-19');
		expect(reply?.text).toContain('2026-06-21');
		expect(reply?.text).toContain('固定 50');
		expect(reply?.text).toContain('/coin pray');
		expect(reply?.text).toContain('允许话题');
		expect(reply?.text).toContain('2026-06-29');
